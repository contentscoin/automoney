...
- **간접구매**(24h): 별도 요율표 `R_a', R_m', R_u'`. 기본 정책은 유저에게도 지급하되 요율을 낮추며, 요율표에서 유저 요율 0으로 설정하면 간접구매 수익은 운영사·총판만 가져갑니다. 간접구매 **집계·금액은 수퍼어드민 전용** 화면에만 노출됩니다(요구사항 7).
...
| 리컨실 통과 | `CONFIRMED`, 원천징수 3.3% 계산, 명세서 PDF 생성 |
| 수퍼어드민 승인 | `APPROVED`, 지급 파일(은행 일괄이체 CSV) 생성 |
| 지급 | `PAID`, 유저 히스토리·텔레그램 알림 |
...
## 5. KYC와 지급 조건
- KYC `APPROVED`가 아니면 정산은 `HELD`. 링크·포스팅·실적 조회는 허용(요구사항 4: "수당을 인정받아 받을 수 있다"는 지급 조건으로 해석).
- 승인 시 HELD 정산을 소급하여 다음 정산 주기에 합산.
- 사업소득 원천징수 3.3% 기본, 사업자 등록 유저는 세금계산서 모드(수퍼어드민 설정).
