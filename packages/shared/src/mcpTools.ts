...
export const MCP_TOOLS: McpToolDef[] = [
  ...
  { name: "post_schedule", description: "예약 발행을 등록합니다(KST, 지터). 위험 툴: confirmed=true 없이는 미리보기만 반환하며, 자동 승인은 confirmAutoApprove=true와 미리보기 confirmationHash도 필요합니다.", scope: "mcp:write", dangerous: true, inputSchema: obj({ spaceId: str("스페이스 _id"), kind: str("주기", { enum: ["ONE_SHOT", "DAILY", "WEEKLY"] }), timeOfDay: str("HH:MM (KST)"), daysOfWeek: { type: "array", items: { type: "integer", minimum: 0, maximum: 6 }, description: "WEEKLY 요일(0=일)" }, runDate: str("ONE_SHOT 실행 일자 YYYY-MM-DD"), jitterMinutes: num("지터(0~120분)", { minimum: 0, maximum: 120 }), text: str("본문(pieceId 지정 시 생략 가능)"), mediaUrls: { type: "array", items: { type: "string" }, description: "미디어 URL" }, contentChannel: str("직접 입력 콘텐츠의 게시 형식(Instagram은 FEED/REEL 필수)", { enum: CHANNEL_ENUM }), linkId: str("마케팅 링크 _id"), pieceId: str("라이브러리 조각 _id"), autoApprove: bool("실행 직전 승인 없이 게시"), clientRequestId: str("확정 호출 재시도 멱등 키(영문·숫자·_·-, 8~100자)"), confirmationHash: str("자동 승인 예약 미리보기에서 받은 대상·형식·본문·미디어·링크 snapshot 해시"), confirmed: bool("true 일 때만 실제 예약 등록"), confirmAutoApprove: bool("autoApprove=true의 반복 무승인 게시를 별도로 확인") }, ["spaceId", "kind", "timeOfDay"]) },
  { name: "post_publish", description: "즉시 발행 잡을 등록합니다. 위험 툴: confirmed=true 없이는 미리보기만 반환합니다. 잡 id 를 반환하고 job_get 으로 결과를 조회합니다.", scope: "mcp:write", dangerous: true, returnsJob: true, inputSchema: obj({ spaceId: str("스페이스 _id"), text: str("본문(pieceId 지정 시 생략 가능)"), mediaUrls: { type: "array", items: { type: "string" }, description: "미디어 URL" }, contentChannel: str("직접 입력 콘텐츠의 게시 형식(Instagram은 FEED/REEL 필수)", { enum: CHANNEL_ENUM }), linkId: str("마케팅 링크 _id"), pieceId: str("라이브러리 조각 _id"), requireApproval: bool("대시보드/텔레그램 승인 후 게시(기본 true)"), dryRun: bool("실제 게시 없이 절차만 검증"), clientRequestId: str("재시도 멱등 키(영문·숫자·_·-, 8~100자)"), confirmationHash: str("무승인 게시 미리보기에서 받은 대상·형식·본문·미디어·링크 snapshot 해시"), confirmed: bool("true 일 때만 실제 등록") }, ["spaceId"]) },
