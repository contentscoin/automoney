...
function PublishPageInner() {
  ...
  const submit = async () => { try { const jobId = await enqueue({ spaceId: spaceId as Id<"spaces">, text, mediaUrls, linkId: (effectiveLinkId || undefined) as Id<"marketingLinks"> | undefined, pieceId: (pieceId || undefined) as Id<"contentPieces"> | undefined, requireApproval: true, dryRun }); setMsg(`게시 검토 작업을 등록했습니다. 작업 결과에서 승인하세요. (${jobId})`); } catch (e) { setMsg(errorMessage(e)); } };
  return <div className="flex flex-col gap-6"><div><h1 className="text-xl font-bold">게시하기</h1><p className="text-sm text-stone-500">콘텐츠, 게시 계정, 링크와 미디어를 확인한 뒤 승인 대기 작업으로 등록합니다.</p></div>
    ...
    <section className="card grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><label className="label">콘텐츠</label><select className="input" value={pieceId} onChange={(e) => { setPieceId(e.target.value); setTextOverride(null); setMediaOverride(null); setLinkId(""); setLinkSelectionTouched(false); }}><option value="">직접 입력</option>{library?.map((item) => <option key={item._id} value={item._id}>[{CHANNEL_LABEL[item.channel] ?? item.channel}] {item.caption.slice(0, 70)}{item.mine ? "" : " · 운영 제공"}</option>)}</select></div><div><label className="label">게시 계정</label><select className="input" value={selectedSpace?._id ?? ""} onChange={(e) => setSpaceId(e.target.value)}><option value="">선택</option>{candidates.map((space) => <option key={space._id} value={space._id}>[{PLATFORM_LABEL[space.platform]}] {space.name}{space.handle ? ` @${space.handle}` : ""}</option>)}</select></div><div><label className="label">마케팅 링크</label><select className="input" value={effectiveLinkId} onChange={(e) => { setLinkId(e.target.value); setLinkSelectionTouched(true); }}><option value="">없음</option>{matchingLinks.map((link) => <option key={link._id} value={link._id}>{link.product?.name ?? link.shortCode}{["MOCK", "DEMO"].includes((link as { origin?: string }).origin ?? "") ? " · 데모" : ""}</option>)}</select>{piece?.productId && links !== undefined && matchingLinks.length === 0 && <p className="mt-1 text-xs text-amber-700">이 상품의 활성 링크가 없습니다. 콘텐츠 제작실에서 먼저 발급하세요.</p>}</div><div className="sm:col-span-2"><label className="label">본문</label><textarea className="input" rows={6} value={text} onChange={(e) => setTextOverride(e.target.value)} /></div><div className="sm:col-span-2"><label className="label">HTTPS 이미지·영상 URL</label><input className="input" value={media} onChange={(e) => setMediaOverride(e.target.value)} placeholder="여러 개는 공백으로 구분" /></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={dryRun} onChange={(e) => setDryRun(e.target.checked)} />테스트 실행(실제 게시 안 함)</label></section>
