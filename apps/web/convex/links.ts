...
export const issue = action({
  ...
  handler: async (ctx, args): Promise<{ linkId: string; shortCode: string; trackingCode: string; existed: boolean }> => {
    const userId = await getAuthUserId(ctx);
    if (!userId) fail("UNAUTHENTICATED", "로그인이 필요합니다.");
    const prep = await ctx.runQuery(internal.links.prepareIssue, { userId, productId: args.productId });
    if (prep.existing) {
      return {
        linkId: prep.existing._id,
        shortCode: prep.existing.shortCode,
        trackingCode: prep.existing.trackingCode,
        existed: true,
      };
    }
    let issued;
