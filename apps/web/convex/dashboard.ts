...
export async function userSummaryFor(ctx: QueryCtx, user: Doc<"users">) {
...
export const userSummary = query({
  args: {},
  handler: async (ctx) => {
    return await userSummaryFor(ctx, await requireUser(ctx));
  },
});

/** 총판 대시보드: 하부 유저별 이번 달 DIRECT 실적. 간접·차액은 M2. */
const adminSummaryArgs = { month: v.optional(v.string()) };

export async function adminSummaryFor(ctx: QueryCtx, actor: Doc<"users">, args: ObjectType<typeof adminSummaryArgs>) {
...
export const adminSummary = query({
  args: adminSummaryArgs,
  handler: async (ctx, args) => {
    return await adminSummaryFor(ctx, await requireAdminOrSuper(ctx), args);
  },
});

/** 수퍼어드민: 전체 집계(간접 포함), 대기 KYC, 유저 수. */
const superSummaryArgs = { month: v.optional(v.string()) };

export async function superSummaryFor(ctx: QueryCtx, args: ObjectType<typeof superSummaryArgs>) {
  const month = args.month ?? kstMonth(Date.now());
  const rate = await getDefaultUserRateBps(ctx);
...
export const superSummary = query({
  args: superSummaryArgs,
  handler: async (ctx, args) => {
    await requireSuperAdmin(ctx);
    return await superSummaryFor(ctx, args);
  },
});
